	...
	<?php if ( class_exists( 'ReduxFramework_extension_wbc_importer' ) ) :
				...
				<div class="imi-col-sm-12">
					<div class="imi-box imi-box-importer">
						<div class="imi-box-head">
							<?php esc_html_e('Warning','framework'); ?>
						</div>
		...
		<div class="imi-row">
			<div class="imi-col-sm-12">
				<div class="imi-box imi-box-importer">
					<div class="imi-box-head">
						<?php esc_html_e('Which demo to import?','framework'); ?>
					</div>
					<div class="imi-box-content">
						<?php echo esc_html__('"Demo using page templates" uses WordPress page templates for all pages and "Demo using page builder" have the pages created using the SiteOrigins page builder. You can choose how you want to modify the content of your website as per your need.','framework'); ?>
					</div>
				</div>
			</div>
		</div>
			...
			<div class="theme-browser rendered wp-clearfix">
				<div class="themes wp-clearfix">
			 	...
			 	<?php
					...
					$demo_screen_url 	= IMIC_THEME_PATH.'/framework/imi-admin/theme-options/screen-images/';
					...
					if ( ! empty( $wbc_demo_imports ) ) {
						...
						foreach ( $wbc_demo_imports as $section => $imports ) :
							...
							$output .= '
								...
								<a class="more-details" href="' . $imports['preview'] . '" target="_blank">' . esc_html__( 'Preview', 'framework' ) . '</a>
							...
							if ( $imported == false ) {
								...
								$output .= '
									<div class="wbc-importer-buttons">
										<span class="spinner">'.esc_html__( 'Please Wait...', 'framework' ).'</span>
										<span class="button button-primary importer-button import-demo-data">' . esc_html__( 'Import', 'framework' ) . '</span>
									</div>';
			
							} else {
			
								$output .= '
									<div class="wbc-importer-buttons button-secondary importer-button">' . esc_html__( 'Imported', 'framework' ) . '</div>
									<span class="spinner">' . esc_html__( 'Please Wait...', 'framework' ) . '</span>
									<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">' . esc_html__( 'Re-Import', 'framework' ) . '</div>';
							...
							<div class="imi-lightbox-wrap wp-clearfix">
								<div class="imi-lightbox-contents">
										...
										<div class="imi-lb-content imi-settings">
											...
											<div class="imi-row-import">
												<div class="imi-install-plugins-wrap">
													<h3>' . esc_html__( 'These below plugins are required', 'framework' ) . '</h3>
														...
														if ( is_plugin_active( $plugin['file_path'] ) ) {
															$plugin_action = '
																<div class="row-actions visible active">
																	<span class="activate">
																		<a class="button imi-admin-btn">' . esc_html__( 'Activated', 'framework' ) . '</a>
																	</span>
																</div>
															';
														}
									...
									$output .= '
												...
												<p>' . esc_html__( 'If the process for any plugin activation keeps loading for more than 5 minutes then refresh this page, click import button again and activate the plugin again.', 'framework' ) . '</p>
												
												</div> <!-- end imi-plugins-wrap -->
											</div>

											<div class="imi-row-import">
												<!-- <h3>' . esc_html__( 'Import content', 'framework' ) . ' <span>' . esc_html__( '(menus only import by selecting "All")', 'framework' ) . '</span></h3> -->
												<h3>' . esc_html__( 'Import content', 'framework' ) . '</h3>
												<div class="imi-import-content-wrap wp-clearfix">
													<div class="imi-checkbox-wrap imi-all-contents">
														...
														<span>' . esc_html__( 'All', 'framework' ) . '</span>
													</div>
													<div class="imi-checkbox-wrap">
														...
														<span>' . esc_html__( 'Contents', 'framework' ) . '</span>
													</div>
													<div class="imi-checkbox-wrap">
														...
														<span>' . esc_html__( 'Images', 'framework' ) . '</span>
													</div>
													<div class="imi-checkbox-wrap">
														...
														<span>' . esc_html__( 'Widgets', 'framework' ) . '</span>
													</div>
													<div class="imi-checkbox-wrap">
														...
														<span>' . esc_html__( 'Theme options', 'framework' ) . '</span>
													</div>
													<div class="imi-checkbox-wrap">
														...
														<span>' . esc_html__( 'Sliders', 'framework' ) . '</span>
													</div>
												</div>
											</div>

											<div class="imi-row-import">
												<a href="#" class="imi-import-demo-btn">' . esc_html__( 'Import', 'framework' ) . '</a>
											</div>
										</div>
										...
										<div class="imi-lb-content imi-suc-imp-content-wrap">
											<div class="imi-suc-imp-content">
												<a href="' . esc_url( home_url( '/' ) ) . '" target="_blank" title="' . esc_html__( 'Visit Site', 'framework' ) . '">
													<img class="wbc_image" src="' . esc_attr( esc_url( $demo_screen_url . $imports['directory'] . '/' . $imports['image'] ) ) . '">
												</a>
												<div class="imi-suc-imp-t100"><strong>100%</strong>' . esc_html__( 'Demo is Successfully Imported', 'framework' ) . '</div>
												<div class="imi-suc-imp-links">
													<a class="imi-suc-imp-links-d" href="' . esc_url( self_admin_url( 'admin.php?page=imi-admin-welcome' ) ) . '">' . esc_html__( 'Native Church Dashboard', 'framework' ) . '</a>
													<a class="imi-suc-imp-links-t" href="' . esc_url( self_admin_url( 'admin.php?page=_options' ) ) . '">' . esc_html__( 'Theme Options', 'framework' ) . '</a>
													<a class="imi-suc-imp-links-v" href="' . esc_url( home_url( '/' ) ) . '" target="_blank">' . esc_html__( 'Visit Site', 'framework' ) . '</a>
												</div>
												<div class="imi-start">
													<span class="imi-start-message">' . esc_html__( 'Please don’t refresh the page until import is done completely, import time is related to your host configuration and it may take upto 15 minutes.', 'framework' ) . '</span>
												</div>
											</div>
										</div>
