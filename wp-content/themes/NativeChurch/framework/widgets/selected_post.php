...
class selected_post extends WP_Widget {
	...
	public function __construct() {
		 $widget_ops = array('description' => __( 'Display latest and selected post of different post type.','framework') );
         parent::__construct(false, $name = __('Selected Post','framework'), $widget_ops);
	}
	...
	?>
       ...
       <p>
            <label for="<?php echo esc_attr($this->get_field_id('type')); ?>"><?php _e('Select Post Type', 'framework'); ?></label>
           ...
           $post_types = get_post_types( array('_builtin' => false,'public'=> true), 'names' ); 
               if(($key = array_search('post', $post_types)) !== false){
                ...
                if(!empty($post_types)){
                    ...
                    echo '<option value="no">'.__('No Post Type Found.','framework').'</option>';
                }
                ?>
            </select> 
        </p>
        <p>
        <label for="<?php echo esc_attr($this->get_field_id('category')); ?>"><?php _e('Post Category', 'framework'); ?></label>
        <input type="hidden" id="selected_cat" value="<?php echo esc_attr($category); ?>">
            <select class="post_cat dynamic_cat" id="<?php echo esc_attr($this->get_field_id('event')); ?>" name="<?php echo esc_attr($this->get_field_name('category')); ?>">
                <option value=""><?php _e('Select Post Category','framework'); ?></option>
                <?php
								switch($type)
								{
									case 'product':
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php _e('Number of posts to show', 'framework'); ?></label>
	...
	public function widget($args, $instance) {
	   ...
	   echo ''.$args['before_widget'];
		...
		if($posts->have_posts()):while($posts->have_posts()):$posts->the_post();
			...
			$postImage = get_the_post_thumbnail( $id, 'full', array('class' => "img-thumbnail") );
				echo '<li class="clearfix">';
						...
						<span class="meta-data"><i class="fa fa-calendar"></i>'.__(' on ', 'framework') . $postDate .'</span>
						</div>
					</li>';					
		...
		echo '</ul>';
			...
			echo __('No ','framework').$type.__(' Found','framework');		
		endif;
