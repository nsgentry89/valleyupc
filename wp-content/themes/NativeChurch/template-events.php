...
function get_future_event($m,$term='')
{
	...
	$month = date_i18n('Y-m', strtotime("+$m month", strtotime($currentEventTime)));
	$events = imic_recur_events('future','nos',$term,$month);
	$data_post['events'] = $events;
	$prev_month = date_i18n('Y-m',strtotime("+$m month", strtotime($currentEventTime)));
    $next_month = date_i18n('Y-m',strtotime("+$m month", strtotime($currentEventTime)));
