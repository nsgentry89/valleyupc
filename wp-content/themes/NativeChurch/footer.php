    ...
    <div class="container">
        <div class="row">
            ...
            if (!empty($options['footer_copyright_text'])) {
                ...
                <p><?php esc_html_e('&copy; ','framework'); echo date('Y '); bloginfo('name'); esc_html_e('. ','framework'); echo wp_kses($options['footer_copyright_text'], $framework_allowed_tags); ?></p>
                <?php echo '</div>';
            }
            ?>
		...
		<script>
			jQuery(document).ready(function() {
               jQuery("a[data-rel^='prettyPhoto']").prettyPhoto({
				  opacity: <?php if(isset($imic_options['prettyphoto_opacity']) && $imic_options['prettyphoto_opacity']!= ""){ echo esc_attr($imic_options['prettyphoto_opacity']); } ?>,
				  social_tools: "",
				  deeplinking: false,
				  allow_resize: <?php if(isset($imic_options['prettyphoto_opt_resize']) && $imic_options['prettyphoto_opt_resize']!= ""){ echo esc_attr($imic_options['prettyphoto_opt_resize']); } else { echo 'true'; } ?>,
				  show_title: <?php if(isset($imic_options['prettyphoto_title']) && $imic_options['prettyphoto_title']== 0){ echo 'true'; } else echo 'false'; ?>,
				  theme: '<?php if(isset($imic_options['prettyphoto_theme']) && $imic_options['prettyphoto_theme']!= ""){ echo esc_attr($imic_options['prettyphoto_theme']); } ?>',
				});
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel"><?php esc_html_e('Your ticket for the ','framework'); echo get_the_title(); ?></h4>
            </div>
                                ...
                                <div class="event-ticket-right-inner">
                                    <div class="row">
                                        <div class="col-md-9 col-sm-9">
                                            <span class="registerant-info">
                                                <?php echo esc_attr($this_actualname).' '.esc_attr($this_lname); ?><br><?php echo esc_attr($this_email); ?>
                                            </span>
                                             <span class="meta-data"><?php esc_html_e('Event','framework'); ?></span>
                                             <h4 id="dy-event-title"><?php echo get_the_title(); ?></h4>
                                        </div>
                                        <div class="col-md-3 col-sm-3">
                                            <span class="ticket-cost"><?php if($event_registration_fee!=0||$event_registration_fee!='') { echo imic_get_currency_symbol(get_option('paypal_currency_options')).$event_registration_fee; } else { esc_html_e('Free','framework'); } ?></span>
                                        </div>
                                    </div>
                                    <div class="event-ticket-info">
                                        <div class="row">
                                            ...
                                            <div class="col">
                                                <p id="dy-event-time"><?php esc_html_e('Starts ','framework'); echo esc_attr(date_i18n(get_option('time_format'),$event_time)); ?></p>
                                            </div>
                                        </div>
                                    </div>
