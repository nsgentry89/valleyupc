    ...
    <?php if(isset($_REQUEST['tx'])){ ?>
    	...
    	<div class="<?php echo esc_attr($pageOptions['class']); ?>" id="content-col"> 
           ...
           $registration_charge = ($event_registration_fee=='')?'Free':imic_get_currency_symbol(get_option('paypal_currency_options')).get_post_meta(get_the_ID(),'imic_event_registration_fee',true);
            ...
            <a href="mailto:<?php echo esc_attr($event_email); ?>" class="btn btn-default" data-placement="bottom" data-toggle="tooltip" data-original-title="<?php _e('Contact us','framework'); ?>" rel="tooltip"><i class="fa fa-envelope"></i></a>
            <?php $imic_event_address=get_post_meta(get_the_ID(), 'imic_event_address', true);
            if(!empty($imic_event_address)){
            echo '<a href="http://maps.google.com/?q='.$imic_event_address.'" class="btn btn-default" data-placement="bottom" data-toggle="tooltip" data-original-title="'.esc_html__('Event Address','framework').'" rel="tooltip" target="_blank"><i class="fa fa-map-marker"></i></a>';   
            }
			...
			if($custom_registration_url != ''){
				...
				if((is_plugin_active('Payment-Imithemes/causes.php'))&&($registration_status==1)) {
					if(is_user_logged_in()) {
						echo '<a href="#" id="donate-popup" class="btn btn-primary donate-paypal" data-toggle="modal" data-target="#PaymentModal">'.esc_html__('Register','framework').'</a>'; }
					else {
						echo '<a href="#" id="login-register" class="btn btn-primary donate-paypal" data-toggle="modal" data-target="#PaymentModal">'.esc_html__('Register','framework').'</a>';
					}
				}
			}
            ?>
